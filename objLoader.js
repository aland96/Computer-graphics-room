class ObjLoader{
	static objToMesh(txt){ 
		...
		while(posB > posA){
			...
			switch(line.charAt(0)){
				...
				case "o":
					var meshName = line.split(" ");
					meshName.shift();
					var objectTxt = txt.split(meshName);
					var data = ObjLoader.parseObjText(objectTxt[1],true);
					var mesh = {name: meshName[0], indices:data[0], vertices:data[1], normals:data[2], textureCoord:data[3]};
				break;
			}
			posA = posB+1;
			posB = txt.indexOf("\n",posA);
	...
	static parseObjText(txt,flipYUV){
		...
		var line,				//Line text from obj file
			...
			cNorm = [],			//Cache Normal array read from obj
			cUV = [],			//Cache UV array read from obj
